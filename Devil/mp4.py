...
async def getdocument(client, message):
    tmp = os.path.join(EX, str(message.chat.id))
    if not os.path.isdir(tmp):
        os.makedirs(tmp)
    doc_path = os.path.join(tmp, str(uuid.uuid4()) + ".mp4")
    vid = await message.reply_text("Uploading to my server and analyzing...", True)
    doc_path = await client.download_media(message=message, file_name=doc_path)
    await vid.edit_text("📷")
    await vid.edit_text("📸")
    await vid.edit_text("🌆")
    ...
    try:
        ...
        return
    await vid.edit_text(
        text=f"""<b>Link :-</b> <code>https://telegra.ph{response[0]}</code>\n\n🖥 Dҽʋ Mҽɳƚισɳ: @mrdevil12 | @devilxtelegraph_bot╰☆☆••| ╰☆☆••| 𝗜𝗺𝗮𝗴𝗲 𝟮 𝗨𝗥𝗟 |••☆☆╮""",
